...
Meteor.methods({
            ...
            _.each(haveJobList, jobID => {
                ...
                if (feedbackIDs) {
                    _.each(feedbackIDs, feedbackID => {
                        // Feedback.update(feedbackID, {$set: {HRState: 'delete'}});
                        Feedback.remove({'_id': feedbackID});// 这样交互很不友好！
                    });
                }
    ...
    deleteResume: function(userID, deletResumeID) {
        let success = false;// 获取失败
        let massage = '';
        let result = {};// 返回结果
        let userInfo = IndividualUsers.findOne({'_id': userID});
        let resumeIDs = userInfo.resumeID;
        if (resumeIDs) {
            _.each(deletResumeID, deletID => {
                let haveResume = false;
                let resumeIndex = '';
                _.each(resumeIDs, (resumeID, index) => {
                    // 看自己的简历信息中是不是有这个简历
                    if (resumeID === deletID) {
                        let feedbackInfo = Feedback.find({'resumeID': deletID}).fetch();
                        _.each(feedbackInfo, info => {
                            // 删除feedback中的信息
                            Feedback.remove({'_id': info._id});
                        });
                        resumeRemove = index;
                        haveResume = true;
                    }
                ...
                if (haveResume) {
                    Resume.remove({'_id': deletID});
                    resumeIDs.splice(resumeIndex,1);
                    success = true;
                }
                else {
                    massage = '用户没有该简历！';
            ...
            IndividualUsers.update(userID, {$set: {'resumeID': resumeIDs}});
        }
        else {
            massage = '该用户没有简历！';
        }
    ...
    getResumes: function(userID, singleResumeID) {
        ...
        if (singleResumeID) {
            let haveResume = false;
            if (userInfo.resumeID && userInfo.resumeID.length) {
                ...
                massage = '用户未添加简历！不能执行此操作，请到个人中心添加简历。';
            }
            
        }
        else {
            let resumeIDs = userInfo.resumeID ? userInfo.resumeID : [];
            if (resumeIDs.length) {
                _.each(resumeIDs, resumeID => {
    ...
    getFeedback: function(userID) {
        ...
        let feedbackData = [];
        let errorCode = null;
        if (feedbakcNum.length) {
            let deleteIDIndex = [];
            _.each(feedbakcNum, (feedID, index) => {
                ...
                if (feedData) {
                    ...
                    deleteIDIndex.push(index);
                    massage = '部分职位已被HR删除。';
                }
            });
            if (deleteIDIndex.length) {
                _.each(deleteIDIndex, index => {
                    feedbakcNum.splice(index, 1);
                    IndividualUsers.update(userID, {$set: {'feedbackID': feedbakcNum}});
                });
            }
    ...
    getApplyResumeList: function(jobID) {
        ...
        _.each(feedbackIDs, feedbackID => {
            ...
            if (resume) {
                resume.feedbackID = feedbackID;
                resume.HRState = feedbackInfo.HRState;
    ...
    updateFeedback: function(feedbackID, HRState) {
        let success = false;// 获取失败
        let massage = '';
        let result = {};// 返回结果
        let feedbackInfo = Feedback.findOne({'_id': feedbackID});
        if (feedbackInfo) {
            success = Feedback.update(feedbackID, {$set: {'HRState': HRState}}) ? true : false;// HR状态更改
            if (!success) {
                massage = '更新失败！';
            }
        }
        else {
            massage = '错误的feedbackID！'
        }
        result = {
            success,
            massage
...
// git log --author="luning04" --pretty=tformat: --numstat | awk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "added lines: %s, removed lines: %s, total lines: %s\n", add, subs, loc }'